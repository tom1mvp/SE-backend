...
class ListAssessmentCategoryView(APIView):
    ...
    def get(self, request):
        ...
        if not assessment_category:
            return Response({'error': 'List assessment categories not found'}, status=status.HTTP_404_NOT_FOUND)
...
class ListGradesView(APIView):
    ...
    def get(self, request):
        ...
        if not grades:
            return Response({'error': 'List grades not found'}, status=status.HTTP_404_NOT_FOUND)
...
class GradesBySubjectView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not name:
            return Response({'error': 'Subject name not found'}, status=status.HTTP_400_BAD_REQUEST)
...
class CreateGradesView(APIView):
    ...
    def post(self, request):
        ...
        if not data or 'subject_id' not in data or 'student_id' not in data or 'category_id' not in data:
            return Response({
                'error': 'no data avaible or required relational IDs (subject, student, category)'
